...
func _ready():
	...
	$Left.texture.set_image(image)
	$Right.texture.set_image(image)
	...
	wasm.load(FileAccess.get_file_as_bytes("res://addons/godot-wasm/doom.wasm"), imports)
...
func draw_screen(offset):
	...
	$Left.texture.update(image)
	$Right.texture.update(image)
